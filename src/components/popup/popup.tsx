...
import calculatePosition, { injectCss } from './utils';

injectCss();

let popupIdCounter = 0;

const MOUNT_ID = 'next-space-popup-root';
const getRootPopup = () => {
  let PopupRoot = document.getElementById(MOUNT_ID);

  if (PopupRoot === null) {
    PopupRoot = document.createElement('div');
    PopupRoot.setAttribute('id', MOUNT_ID);
...
export const Popup = forwardRef<PopupActions, PopupProps>(
  (
    ...
    const popupId = useRef<string>(`next-space-popup-${++popupIdCounter}`);
    ...
    // 有hover不能有overlay
    const hasOverlay = !on.includes('hover');
