...
const HeadCell: React.FunctionComponent<IHeadCellProps> = (props) => {
  ...
  if (columnReordering) {
    const reorderedRowProps: ChildAttributesItem<IHeadCellProps> =
      getDraggableProps(
        key,
        dispatch,
        reorderColumns,
        defaultOptions.css.draggedColumn,
        defaultOptions.css.dragOverColumn
      );
  ...
  const { elementAttributes, content } = getElementCustomization(
    {
      className: classNames(
        defaultOptions.css.theadCell,
        defaultOptions.css.theadBackground,
        isSortingEnabled(sortingMode) ? 'ka-pointer' : ''
      ),
      style: stateStyle,
      scope: 'col',
    },
