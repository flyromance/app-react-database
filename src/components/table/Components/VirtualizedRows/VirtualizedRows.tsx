...
const VirtualizedRows: React.FunctionComponent<ITableBodyProps> = (props) => {
  const { data, dispatch, virtualScrolling } = props;
  ...
  const onFirstRowRendered = (firstRowRef: RefObject<HTMLElement>) => {
    if (
      ...
      firstRowRef.current &&
      virtualScrolling &&
      (!virtualScrolling.itemHeight || !virtualScrolling.tbodyHeight)
    ) {
      const itemHeight = firstRowRef.current.offsetHeight || 40;
      const rootElement: any = firstRowRef.current.closest(
        `.${defaultOptions.css.root}`
      );
      const tbodyHeight = (rootElement && rootElement.offsetHeight) || 600;
      ...
      dispatch({
        ...
        virtualScrolling: newVirtualScrolling,
      });
    }
  };
