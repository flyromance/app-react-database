...
export type BlockType =
  ...
  | 'file'
  | 'code'
  | 'collection_view' // 在 page_block 中插入 database_inline 则创建的这种类型
  | 'collection_view_page' // 在 page_block 中插入 database_full_page 则创建的这种类型
  | string;
...
export interface BasePageBlock extends BaseBlock {
  properties?: {
    title: Decoration[]; // 普通的page，只有这个字段
    [key: string]: Decoration[]; // database中的每条page数据，就有这个字段
  };
  format: {
